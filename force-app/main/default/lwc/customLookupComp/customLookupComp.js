...
import fetchLookupData from '@salesforce/apex/ProductIntrestController.fetchLookupData';
import fetchDefaultRecord from '@salesforce/apex/ProductIntrestController.fetchDefaultRecord';
   ...
   // initial function to populate default selected lookup record if defaultRecordId provided  
    ...
    @wire(fetchLookupData, { searchKey: '$searchKey' , sObjectApiName : '$sObjectApiName' })
     searchResult(value) {
        ...
        if (data) {
            console.log('ChildProdid---',this.childProdId);
            console.log('ChildProdData---',data);
             this.hasRecords = data.length == 0 ? false : true; 
             this.lstResult = JSON.parse(JSON.stringify(data)); 

             this.handelSelectedRecord();
         }
        else if (error) {
...
handelSelectedRecord(event){   
    debugger;
    console.log('SelectedRecordCalled----',this.childProdId);

    var objId;
     if(this.childProdId && !event){
        this.selectedRecord = this.lstResult.find(data => data.Id === this.childProdId); // find selected record from list 
     }else{
        objId = event.Id// get selected record Id 
